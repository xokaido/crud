   ...
   $.extend( altEditor.prototype, {
       ...
       _openEditModal: function ( )
       {
         ...
         for( let key in xcolumns )
          ...
          let fieldValue = eval( 'adata.data()[0].' + columnDefs[ j ].title.toLowerCase()  );

            data += "<div class='form-group'><div class='col-sm-3 col-md-3 col-lg-3 text-right' style='padding-top:7px;'><label for='" +
            columnDefs[j].title + "'>" + columnDefs[j].title +
            ":</label></div><div class='col-sm-9 col-md-9 col-lg-9'><input type='text'  id='" + columnDefs[j].title +
            "' name='" + columnDefs[j].title +
            "' placeholder='" + columnDefs[j].title +
            "' style='overflow:hidden'  class='form-control  form-control-sm' value='" + fieldValue +
       ...
       _openDeleteModal: function ()
       {
         ...
         for( let key in xcolumns )
          ...
          let fieldValue = eval( 'adata.data()[0].' + columnDefs[ i ].title.toLowerCase()  );

            data += "<div class='form-group'><label for='" + columnDefs[i].title + "'>" +
            columnDefs[i].title + " : </label><input  type='hidden'  id='" + columnDefs[i].title +
            "' name='" + columnDefs[i].title + "' placeholder='" + columnDefs[i].title +
            "' style='overflow:hidden'  class='form-control' value='" +
            fieldValue + "' >" + fieldValue + "</input></div>";
       ...
       _deleteRow: function( )
       {
         ...
         $.post( postConfigs[0].deleteURL, { data }, function( r ) {
            $('#altEditor-modal .modal-body').append(message);
            dt.ajax.reload();
         });
       ...
       _addRowData: function()
       {
        ...
        $.post( postConfigs[0].addURL, { data: xdata }, function( r ) {
